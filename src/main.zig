...
const ImageCache = std.StringArrayHashMapUnmanaged(CacheEntry);
...
fn updateViewport(self: *@This(), update: enum { alone, with_main }) !void {
    ...
    const src_w: f32 = @floatFromInt(if (rotated) header.h else header.w);
    const src_h: f32 = @floatFromInt(if (rotated) header.w else header.h);

    // Surface local coordinates
    const dst_w: f32 = @as(f32, @floatFromInt(self.vp.w)) * self.zoom;
    const dst_h: f32 = @as(f32, @floatFromInt(self.vp.h)) * self.zoom;
    const dst_x: f32 = @as(f32, @floatFromInt(self.vp.x)) - (dst_w - @as(f32, @floatFromInt(self.vp.w))) / 2 + @as(f32, @floatFromInt(self.offset_x));
    const dst_y: f32 = @as(f32, @floatFromInt(self.vp.y)) - (dst_h - @as(f32, @floatFromInt(self.vp.h))) / 2 + @as(f32, @floatFromInt(self.offset_y));
...
fn onXdgTopLevelEvent(self: *@This(), _: *shimizu.Connection, _: xdg_shell.xdg_toplevel, event: xdg_shell.xdg_toplevel.Event) !void {
    switch (event) {
        .configure => |configure| {
            ...
            const ww: i32 = @max(configure.width, 320);
            const wh: i32 = @max(configure.height, 320);
            switch (self.image_buffer.backing) {
                .uninitialized, .pixel => {},
                .image => |data| {
                    const dst_aspect: f32 = @as(f32, @floatFromInt(data.image.header.w)) / @as(f32, @floatFromInt(data.image.header.h));
                    const src_aspect: f32 = @as(f32, @floatFromInt(configure.width)) / @as(f32, @floatFromInt(configure.height));
                    if (src_aspect > dst_aspect) {
                        vp.h = @intCast(wh);
                        vp.w = @intFromFloat(@round(@as(f32, @floatFromInt(vp.h)) * dst_aspect));
                        vp.x = @intFromFloat(@round(@as(f32, @floatFromInt(ww - vp.w)) / 2));
                    } else {
                        vp.w = @intCast(ww);
                        vp.h = @intFromFloat(@round(@as(f32, @floatFromInt(vp.w)) * dst_aspect));
                        vp.y = @intFromFloat(@round(@as(f32, @floatFromInt(wh - vp.h)) / 2));
                    }
                },
            }
            self.vp = vp;
...
fn run(self: *@This(), cache: ImageCache) !void {
    var title_buf: [256]u8 = undefined;
    var title: std.Io.Writer = .fixed(&title_buf);

    for (cache.entries.items(.key), cache.entries.items(.value)) |k, *v| {
        if (std.fs.cwd().openFile(k, .{})) |f| {
            var buf: [4096]u8 = undefined;
            var reader = f.readerStreaming(&buf);
            var allocating: std.Io.Writer.Allocating = .init(self.allocator);
            errdefer allocating.deinit();
            const hdr = try qoi.decode(&reader.interface, &allocating.writer);
            v.* = .{ .image = .{ .header = hdr, .pixels = try allocating.toOwnedSlice() } };
        } else |_| {}
    }

    const active_image: usize = 0;
    try title.print("swiv <{s}>\x00", .{cache.keys()[active_image]});
...
fn getConverters(gpa: std.mem.Allocator, arena: std.mem.Allocator) ![]const Converter {
...
pub fn main() !void {
    ...
    while (args.next()) |arg| try cache.put(gpa.allocator(), arg, .pending);
    ...
    const converters = try getConverters(gpa.allocator(), arena.allocator());
    for (converters) |c| {
        log.info("{s}", .{c.name});
    }
